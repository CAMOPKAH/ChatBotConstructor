...
async def create_user(user_id: str = Form(...), platform: str = Form(...), db: Session = Depends(get_db)):
...
async def view_trace(request: Request, user_id: str = None, db: Session = Depends(get_db)):
